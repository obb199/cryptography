...
unsigned long long geracao_chave_privada(long long); // A partir de uma seed, gera um número aleatório que servirá de chave privada
...
int main(void){
    ...
    usuario1.chave_privada = geracao_chave_privada(primo);
    usuario1.chave_publica = geracao_chave_publica(ponto_Gx, usuario1.chave_privada, primo);

    usuario2.chave_privada = geracao_chave_privada(primo);
...
unsigned long long geracao_chave_privada(long long primo){ // Gera um valor aleatória, com base em uma semente, que será usado como chave privada
    Sleep(1100);
    srand(time(NULL));
    return rand()%primo;
